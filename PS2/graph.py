...
class WeightedEdge(Edge):
    """Represents a weighted edge, a child/subclass of parent class Edge, in the dictionary. Includes a source,
    a destination, and two weights--one for the total distance from source to destination and another for the distance
    from source to destination that must be traversed outdoors."""
    def __init__(self, src, dest, total_distance, outdoor_distance):
        # first call the parent class constructor
        Edge.__init__(self, src, dest)
        # then set the other two subclass attributes to the parameters
...
class Digraph(object):
    ...
    def add_edge(self, edge):
        ...
        if not (src in self.nodes and dest in self.nodes) or not (src in self.edges and dest in self.edges):
